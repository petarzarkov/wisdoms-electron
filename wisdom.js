const getRandomEmote = async (size = "1x") => {
    const res = await (await window.fetch("https://api.betterttv.net/3/emotes/shared/trending?offset=0&limit=50")).json();
...
const getWisdom = async (lang = "en") => {
    const res = await (await window.fetch(`https://wisdoms-app.herokuapp.com/api/getWisdom?lang=${lang}`)).json();
    ...
    if (wisdomPlaceholder) {
        wisdomPlaceholder.innerHTML = res.error ? res.error :
            ...
            <div class="tooltip">
            <button onclick="copyToClip('${res.wisdom.replace(/'/g, "")}')" style="margin: 5px; border: 0; color: green; background-color: transparent; cursor: pointer;" class="fa fa-copy">
